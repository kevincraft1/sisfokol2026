        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody>
                ...
                foreach ($users as $u): ?>
                    ...
                    <?php
                    // Logika untuk menonaktifkan tombol Hapus
                    $isDiriSendiri = ($u['id'] == session()->get('id'));
                    $isAdmin = ($u['role'] == 'admin');
                    $disabledDelete = ($isDiriSendiri || $isAdmin);
                    ?>

                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                        <td class="py-4 px-6 text-gray-600"><?= $no++; ?></td>
                        <td class="py-4 px-6 flex items-center space-x-3">
                            <img src="https://ui-avatars.com/api/?name=<?= urlencode($u['nama_lengkap']); ?>&background=random&color=fff" class="w-10 h-10 rounded-full">
                            <div>
                                <p class="font-bold text-gray-800"><?= $u['nama_lengkap']; ?> <?= $isDiriSendiri ? '<span class="text-xs text-indigo-500">(Anda)</span>' : ''; ?></p>
                        ...
                        <td class="py-4 px-6 flex justify-center space-x-2">
                            ...
                            <button type="button"
                                <?= !$disabledDelete ? "onclick=\"confirmDelete('" . base_url('panel/users/delete/' . $u['id']) . "')\"" : "" ?>
                                class="px-3 py-1.5 rounded transition text-xs font-medium 
                            <?= $disabledDelete ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-red-50 text-red-600 hover:bg-red-100'; ?>"
                                <?= $disabledDelete ? 'disabled title="Tidak dapat dihapus"' : 'title="Hapus"'; ?>>
